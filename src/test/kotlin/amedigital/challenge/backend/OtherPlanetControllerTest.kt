...
@Sql("/data/remove.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
class OtherPlanetControllerTest {
    ...
    inner class AddPlanet {
        ...
        @Sql("/data/insert_one_planet.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
    ...
    inner class SearchByName {
        ...
        private fun getPlanetByName(planetName: Any) =
            mockMvc.get("$baseUri/search") {
                contentType = MediaType.APPLICATION_JSON
                accept = MediaType.APPLICATION_JSON
                param("name", planetName.toString())
            }
    }
